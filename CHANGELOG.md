...
- **One-liner installer:** `curl -fsSL https://raw.githubusercontent.com/doriangironde/ax/main/install.sh | sh` installs the latest release into `~/.ax/bin` with platform detection, sha256 verification, and PATH setup (`install.sh` in the repo root; pin versions with `AX_VERSION`, choose an install directory with `AX_INSTALL_DIR`).
- **ax-named release assets:** GitHub Release tarballs and checksums are `ax-<platform>-<arch>.tar.gz` (macos arm64 is `ax-macos-arm64`, linux arm64 is `ax-linux-aarch64`), generated by `release.yml` and consumed by the installer.
- **Downloads from GitHub Releases:** the release workflow no longer mirrors tarballs to the upstream CDN (the fork has no Vercel R2 credentials); the GitHub Release is the download source.
- **Documentation sweep:** the README install instructions and user-facing command copy say `ax` (`ax login codex|grok`, `ax status`, `ax doctor`, `/mcp` notes), while compatibility surfaces (`~/.fx`, `FX_*`, `fx.sh` docs links, the upstream repo) stay documented as intentional.
