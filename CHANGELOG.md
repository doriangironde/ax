...
- **Provider commands and picker:** `ax provider` accepts registered custom provider names, `ax models` lists their models with the saved selection, and the interactive `/login` **Model provider** stage shows registered custom providers after the built-in routes, marks the active one, and activates a picked provider in place. A custom provider without a key prompts inline for one and saves it to `providers.json` instead of requiring an environment variable.
- **Login menu providers:** `/login` opens the setup hub and its **Model provider** stage lists registered custom providers and unregistered presets after the built-in routes, so picking one needs no extra step; choosing a preset registers it first, keyless providers switch straight away, the active provider row reads `current`, and unregistered presets carry no description. Provider rows use curated display names (OpenCode Go, OpenRouter, Ollama), including the API-key entry heading and the switch notice.
- **Upstream sync:** The fork base moved from upstream v0.0.5 to v0.0.6. Every upstream change ships (setup hub, per-provider model preferences, exact-action auto review, remote MCP servers, foreground command timeouts, and the rest of the v0.0.6 notes below), with the custom provider cloud, presets, and the ax identity re-applied on top.
- **Upstream sync round 2:** The fork tracks the newest upstream main, shipping the durable provider usage ledger, the inline skill picker with `$` suggestions, natural-language capability discovery, the streamlined sign-in hub, and the rest of the newer upstream backlog, with the custom provider cloud, presets, and the ax identity re-applied on top. Custom provider completions report possibly-billed usage instead of an immediate reference; the repo setup for the release lane and the mock-server e2e follow in the next round.
- **Provider model refresh:** `ax provider refresh <name>` fetches a registered custom provider's live `/models` catalog and merges it into `providers.json`: newly advertised models are added, existing models keep their declared metadata, and models the endpoint dropped are removed except the provider's currently selected model, which is retained so open sessions stay selectable.
