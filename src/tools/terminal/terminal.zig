...
/// intentionally absent; Core derives them from the current ax session.
...
pub fn mapAuthorizedResult(
    ...
    mapped.status_detail = switch (code) {
        ...
        .authority_retired => try alloc.dupe(
            u8,
            "saved terminal authority is from an older ax version; start a new terminal",
        ),
        else => return result,
    };
    return mapped;
...
test "terminal result mapper adds detail for actionable failures" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .expected_detail = "saved terminal authority is from an older ax version; start a new terminal",
        },
