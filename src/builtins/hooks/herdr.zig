...
//! ignored so the integration cannot block or terminate an ax session.
...
pub const Client = struct {
    ...
    /// Report ax's session id so herdr can associate the pane with a resumable
    ...
    /// Name the pane/agent so herdr lists ax even while idle. Best-effort;
...
test "report_agent serializes a single newline-delimited json line" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ax\",\"agent\":\"ax\",\"state\":\"working\"," ++
...
test "report_agent omits custom_status when null" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"1\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ax\",\"agent\":\"ax\",\"state\":\"idle\"}}\n",
        out.written(),
    );
}
...
test "report_agent escapes pane id" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"2\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"pane\\\"x\"," ++
            "\"source\":\"custom:ax\",\"agent\":\"ax\",\"state\":\"blocked\"}}\n",
        out.written(),
    );
}
...
test "pane.rename labels the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 4, "pane.rename", "pane_id", "w1:p1", "label", "ax");
    try std.testing.expectEqualStrings(
        "{\"id\":\"4\",\"method\":\"pane.rename\",\"params\":{\"pane_id\":\"w1:p1\",\"label\":\"ax\"}}\n",
        out.written(),
    );
}
...
test "agent.rename names the agent" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 5, "agent.rename", "target", "w1:p1", "name", "ax");
    try std.testing.expectEqualStrings(
        "{\"id\":\"5\",\"method\":\"agent.rename\",\"params\":{\"target\":\"w1:p1\",\"name\":\"ax\"}}\n",
        out.written(),
    );
}
...
test "clear_agent_authority removes ax from the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeClearAuthority(&out.writer, 7, "w1:p1");
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.clear_agent_authority\",\"params\":{\"pane_id\":\"w1:p1\",\"source\":\"custom:ax\"}}\n",
        out.written(),
    );
}
...
test "report_agent_session serializes session identity" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"3\",\"method\":\"pane.report_agent_session\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ax\",\"agent\":\"ax\",\"agent_session_id\":\"session-42\"}}\n",
        out.written(),
    );
}
