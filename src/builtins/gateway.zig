...
/// An ax login can reach several teams, so `/v1/credits` rejects it outright
...
test "oauth transport user agent uses the product version" {
    try std.testing.expect(std.mem.startsWith(u8, gateway_client.user_agent, "ax/"));
    try std.testing.expect(gateway_client.user_agent.len > "ax/".len);
...
test "model catalog GET sends ax user agent without attribution headers" {
    var fixture = try gateway_client.TestModelCatalogFixture.init();
    defer fixture.deinit();
    try fixture.start();
    try std.testing.expect(fixture.waitForAcceptStart(5000));
