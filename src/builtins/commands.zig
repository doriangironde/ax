...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        ...
        .options = &.{
            ...
            .{ .flag = "--yolo", .description = "Disable ax permission checks" },
    ...
    .{
        ...
        .summary = "Show local ax token usage and spend",
        .options = &.{
            .{ .flag = "--period <24h|7d|30d>", .description = "Select a rolling window (default: 30d)" },
            json_option,
        },
        .details = &.{
            "Reports only usage recorded by ax on this machine.",
    ...
    .{
        ...
        .summary = "Upgrade 𝒂x on the selected release channel",
        .options = &.{
...
pub const top_level_flags = [_]TopLevelFlag{
    ...
    .{
        .usage = "-v, --version",
        .description = "Print the 𝒂x version and exit",
    },
};

pub const top_level_examples = [_]TopLevelExample{
    .{ .command = "ax", .description = "Start a fresh interactive session" },
    .{ .command = "ax ask \"Explain the changes in this repository\"", .description = "Run one request and exit" },
    .{ .command = "ax session resume last", .description = "Continue the latest session for this workspace" },
    .{ .command = "ax status --json", .description = "Inspect the current configuration as JSON" },
};

pub const top_level_notes = [_][]const u8{
    "Run `ax <command> --help` for command-specific options and examples.",
    "Run `/help` inside an interactive session for slash commands.",
};

pub const top_level_resources = [_]TopLevelResource{
    .{ .label = "Learn more about 𝒂x:", .value = "https://fx.sh/docs", .link = true },
    .{ .label = "Report a problem:", .value = "run `/feedback` inside 𝒂x" },
};
...
pub const top_level_registry = TopLevelRegistry{
    ...
    .interactive_hint = "𝒂x starts an interactive session by default. Use `ax ask` to run one noninteractive request.",
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .usage, .command = "/usage", .aliases = &.{"/cost"}, .help_entry = "/usage (/cost)", .completion_description = "show local ax tokens, models, and spend", .presentation_category = .account },
    ...
    .{ .kind = .permissions, .command = "/permissions", .help_entry = "/permissions [ask|auto|yolo|reset]", .completion_description = "choose what ax is allowed to do", .presentation_category = .security, .show_in_welcome = true, .has_args = true, .accepts_payload = true },
    ...
    .{ .kind = .feedback, .command = "/feedback", .help_entry = "/feedback", .completion_description = "open the ax feedback form", .presentation_category = .product, .show_in_welcome = true },
    ...
    .{ .kind = .version, .command = "/version", .help_entry = "/version", .completion_description = "show the ax version", .presentation_category = .general },
