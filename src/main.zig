...
const App = struct {
    ...
    pub fn providerSet(self: *App) provider_set.Set {
        ...
        // Custom provider routes are per-selection: the transport is built
        // from the live registry entry and the static catalog context points
        // at the app field so worker-thread fetches stay valid.
        const selection = self.provider_selection.selection();
        if (selection.provider == .custom and !host_target.is_wasm) {
            if (self.selectionCustomEntry()) |entry| {
                self.refreshCustomProviderState();
                const context_ptr: *const custom_providers.StaticCatalogContext =
                    &(self.custom_catalog_context orelse return providers);
                providers.custom = builtin_providers.customBundle(entry, context_ptr);
            }
        }
        return providers;
    }
