...
const onboarding_note = "   ⚠︎ Note: ax is experimental and defaults to auto mode.";
...
fn composeOnboardingPickerRow(
    ...
    const label = switch (source_row_index) {
        0 => "   Welcome to ax",
        1 => "",
        2 => "   ax can access AI models with an account, subscription, or API key.",
...
test "auth onboarding composes the welcome copy and setup choices" {
    ...
    try std.testing.expect(std.mem.find(u8, screen.items, "Welcome to ax") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "ax can access AI models with an account, subscription, or API key") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "You can change this anytime with /setup.") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "⚠︎ Note: ax is experimental and defaults to auto mode. \x1b]8;id=fx-onboarding;https://fx.sh/docs/stability\x1b\\\x1b[4mLearn more\x1b[24m\x1b]8;;\x1b\\") != null);
    ...
    try std.testing.expect(std.mem.find(u8, body_row.items, "ax can access AI models") != null);
...
test "partially visible auth picker shows a source window without duplicates" {
    ...
    try std.testing.expect(std.mem.find(u8, first_source.items, "ax login") == null);

    var selected_source = try composeAuthPickerRow(alloc, view, 3, 4, 80);
    defer selected_source.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, selected_source.items, "ax login") != null);
    ...
    try std.testing.expect(std.mem.find(u8, scrolled_first.items, "ax login") != null);
