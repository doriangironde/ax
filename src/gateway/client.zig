...
/// Identifies ax on every AI Gateway request; the zig std.http default
/// (`zig/<version> (std.http)`) is never sent to the gateway.
pub const user_agent = "ax/" ++ build_options.app_version;
...
pub fn postGatewayCompletion(
    ...
    while (attempt < retry_count) : (attempt += 1) {
        ...
        const extra_headers = [_]std.http.Header{
            .{ .name = "HTTP-Referer", .value = "https://github.com/vercel-labs/ax" },
            .{ .name = "X-Title", .value = "ax" },
...
fn gatewayExtraHeaders(
    ...
    var len: usize = 0;
    buf[len] = .{ .name = "HTTP-Referer", .value = "https://github.com/vercel-labs/ax" };
    len += 1;
    buf[len] = .{ .name = "X-Title", .value = "ax" };
    len += 1;
...
test "gateway chat request sends ax user agent and attribution headers" {
    ...
    try std.testing.expectEqualStrings("https://github.com/vercel-labs/ax", fixture.capturedHeaderValue("http-referer").?);
    try std.testing.expectEqualStrings("ax", fixture.capturedHeaderValue("x-title").?);
