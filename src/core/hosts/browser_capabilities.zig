...
pub const model_context =
    "Runtime capabilities: this is the embedded browser version of ax, not locally installed ax. " ++
    ...
    "If the user asks for external web content, explain this limitation immediately and say that locally installed ax provides the full tool suite. " ++
...
test "browser model context refuses unavailable network workarounds" {
    ...
    try std.testing.expect(std.mem.find(u8, model_context, "locally installed ax provides the full tool suite") != null);
}
