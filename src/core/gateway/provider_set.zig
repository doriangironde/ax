...
pub const Set = struct {
    ...
    codex: Bundle,
    grok: Bundle,
    /// Route for the currently selected registered custom provider, if any.
    /// The app fills this from its live registry; an empty bundle yields
    /// unavailable streams and no reviewer.
    custom: Bundle = .{},
    ...
    pub fn deferredUsageProviders(self: Set) generation_usage_provider.Set {
        return .{
            ...
            .custom = self.custom.deferred_usage,
        };
    }
};
...
test "provider set selects each provider's complete route" {
    ...
    try std.testing.expect(providers.select(.custom).agent_stream == null);
    try std.testing.expect(providers.select(.custom).permission_reviewer == null);
