...
test "processQueuedPrompt refreshes ax login credential before gateway request" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{.{ .content = "Done." }};
    var gateway = FakeGateway.init(alloc, &completions);
    defer gateway.deinit();
...
test "processQueuedPrompt refreshes and retries once after ax login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt does not retry a second ax login 401" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
...
test "processQueuedPrompt keeps the selected ax login credential when forced refresh is unavailable" {
    const alloc = std.testing.allocator;
    const completions = [_]FakeCompletion{
        .{
            .status = .unauthorized,
