...
pub fn Runtime(comptime App: type) type {
    return struct {
            ...
            const result = login_flow.logout(app.alloc, app.auth.oauthTransport()) catch |err| switch (err) {
                error.SessionDeleteFailed => {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .@"error",
                        .body = "Could not complete ax logout. The current source is unchanged.",
                    });
                    return;
                },
            };
        ...
        fn applyLogoutResult(app: *App, result: login_flow.LogoutResult) !void {
            ...
            // A remembered source always wins resolution, so an active ax login
            ...
            try writeAuthNotice(app, if (result.local_durability_failed)
                .{
                    .topic = "auth",
                    .tone = .warning,
                    .body = "Could not confirm durable ax logout. The active source was recalculated.",
                }
            else if (result.session_deleted)
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "Signed out of ax.",
                }
            else
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "No ax login session found.",
                });
                        ...
                        .vercel => |*selection| {
                            if (!try selectCredentialSource(app, .fx_login)) {
                                ...
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
                                    .tone = .@"error",
                                    .body = "Signed in, but the ax login credential could not be loaded.",
                                });
                                return;
                            }
            ...
            var credential = resolution.credential orelse {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .body = if (intent == .post_oauth)
                        ...
                        "Run ax login codex, then try switching again."
                    else if (target == .grok)
                        "Run ax login grok, then try switching again."
                    else
                        credentials.missing_interactive_credential_message,
                }, true);
                return;
        ...
        fn beginTeamPicker(app: *App) !void {
            ...
            var selection = login_flow.loadTeamSelection(app.alloc, app.auth.oauthTransport()) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.NoSession => "The ax login session is no longer available. Sign in to change teams.",
        ...
        fn applyTeamChoice(app: *App, index: usize) !void {
            ...
            var selected_team = selection.select(app.alloc, index) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.SessionChanged, error.NoSession => "The ax login session changed before the team could be saved.",
            ...
            if (app.auth.credentialSource() == .fx_login) {
                ...
                try app.writeDomainNotice(.{
                    .topic = "auth",
                    .tone = .@"error",
                    .body = "Changed the Vercel team, but the ax login credential could not be loaded.",
                }, true);
                return;
            }
            rememberCredentialSource(app, .fx_login);
        ...
        fn writeLoginError(app: *App, source: credentials.Source, err: anyerror) !void {
            const notice: types.SemanticNotice = if (source == .chatgpt_subscription)
                ...
                error.ClientIdMissing => .{ .topic = "auth", .tone = .@"error", .body = "ax login is not configured yet. The current credential is unchanged." },
...
const TestAuth = struct {
    ...
    fn customProviderKeyEntryActive(_: *const TestAuth) bool {
        return false;
    }

    fn customKeyEntryProviderName(_: *const TestAuth) []const u8 {
        return "";
    }

    fn takeApiKeyBuffer(_: *TestAuth, _: std.mem.Allocator) []u8 {
        return &.{};
    }

    fn exitApiKeyStage(_: *TestAuth, _: std.mem.Allocator, _: auth_runtime.ApiKeyExitReason) void {}

    fn openProviderPicker(
        self: *TestAuth,
        _: std.mem.Allocator,
        provider: model_provider.ProviderId,
        _: []const u8,
    ...
    fn openCustomProviderKeyPicker(
        self: *TestAuth,
        _: std.mem.Allocator,
        _: []const u8,
    ) void {
        self.picker_opened = true;
        self.picker_provider = .custom;
    }

    fn adoptCredential(_: *TestAuth, _: std.mem.Allocator, _: *credentials.Credential) bool {
        return true;
    }
...
test "team change from an environment source activates and remembers ax login" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "team change on an active ax login updates and remembers the selected team" {
    var app: TestApp = .{};
    defer app.deinit();
...
test "successful direct login remembers ax login after activation" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "logout result reconciles live auth and renders only sanitized notices" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache_warmup_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Signed out of ax.") != null);
...
test "logout durability failure still reconciles live auth" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.auth.logout_reconcile_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Could not confirm durable ax logout.") != null);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, login_flow.remote_revocation_warning) != null);
...
test "prompt credential refresh failure is recoverable and detail-free" {
    ...
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "ax login credential refresh failed.") != null);
