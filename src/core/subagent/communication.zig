...
const parent_retention_gap_id = "ax-retention-gap";
...
pub fn approvalIdentityFingerprint(input: ApprovalInput) [32]u8 {
    ...
    hash.update("ax.subagent.approval-identity.v2\x00");
...
pub fn preparedRequestFingerprint(request: permission_request.PermissionRequest) [32]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.prepared-permission.v1\x00");
...
pub fn stableDeliveryId(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.delivery.v1\x00");
...
pub fn stableIntervalDeliveryId(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.interval-delivery.v1\x00");
    hashString(&hash, source_id);
    hashString(&hash, work_id);
...
pub fn stableToolActivityId(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.tool-activity.v1\x00");
    hashString(&hash, source_id);
    hashString(&hash, work_id);
...
pub fn stableApprovalId(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.approval.v1\x00");
...
pub fn relationshipPreparedFingerprint(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ax.subagent.relationship-approval.v1\x00");
