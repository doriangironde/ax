...
/// The instant the Thinking clock reads. While ax waits on user input the
...
/// Tracks whether ax is waiting on user input (approval prompt or question)
...
/// producing output ax cannot show yet, typically a large tool payload. The row
