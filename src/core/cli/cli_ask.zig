...
/// Resume selector parsed from ax ask --resume.
...
const AskContext = struct {
    ...
    fn configureNotifications(
        ...
        if (attention_required) {
            try self.lifecycle_runtime.registerAttentionRequired(.{
                .name = "ax.sound.attention_required",
                .ctx = self,
    ...
    fn initializeSessionStores(self: *AskContext) !void {
        var store = session_store.Store.init(self.alloc, self.workspace_root) catch |err| {
            ...
            try self.writeStderr("ax ask: warning: session persistence unavailable; error=");
    ...
    fn toolContext(self: *AskContext) tool_runtime.Context {
        const provider_capabilities = self.providerSet().select(self.provider).capabilities;
    ...
    fn admissionAutoClassifier(self: *AskContext) permission_auto_classifier.Classifier {
        ...
        const provider = self.providerSet().select(self.provider).permission_reviewer orelse
    ...
    /// The provider set with the live custom bundle injected when the
    /// selection is a registered custom provider. The catalog context points
    /// at the registry field on this context, so worker-thread fetches stay
    /// valid.
    fn providerSet(self: *const AskContext) provider_set.Set {
        var set = self.cfg.provider_set;
        if (self.provider == .custom and self.custom_provider.len > 0) {
            if (self.custom_catalog_context) |*context| {
                if (self.custom_registry.find(self.custom_provider)) |entry| {
                    set.custom = builtin_providers.customBundle(entry, context);
                }
            }
        }
        return set;
    ...
    fn agentStreamProvider(self: *const AskContext) agent_stream_provider.Provider {
        return self.providerSet().select(self.provider).agent_stream_or_unavailable();
    }
...
fn writeAskUsage(deps: RunDeps, usage: []const u8) !void {
    try deps.write_stderr(deps.stderr_ctx, "usage: ax ");
...
fn runWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !u8 {
    ...
    var options = parseOptionsWithStdin(alloc, args, deps.stdin_source) catch |err| switch (err) {
        error.MissingPrompt => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "ax ask: missing prompt\n");
        ...
        error.NoSaveResumeConflict => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "ax ask: --no-save cannot be used with --resume or --resume-id\n");
            try writeAskUsage(deps, cfg.command_usage);
            return 1;
        },
        error.PromptResourceLimitExceeded => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "ax ask: prompt exceeds the local input safety limit\n");
            return 1;
        },
        error.PromptInputReadFailed => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "ax ask: failed to read prompt from stdin\n");
            return 1;
        },
        ...
        error.InvalidPromptText => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "ax ask: prompt must be valid UTF-8 and contain no NUL bytes\n");
            return 1;
        },
        else => return err,
    };
    ...
    const result = runPromptInternal(alloc, options.prompt, options.permission_override, effective_cfg, .{
        ...
        if (err == error.OneOffSessionNotResumable and !options.json_output) {
            try deps.write_stderr(
                deps.stderr_ctx,
                "ax ask: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
            );
            return 1;
        }
        if (!options.json_output) {
            const notice = askErrorNotice(err) orelse return err;
            try deps.write_stderr(deps.stderr_ctx, "ax ask: ");
    ...
    for (options.image_paths.items) |image_path| {
        const attachment = image_attachments.loadUserImageAttachment(
            ...
            if (options.json_output) {
                ...
                try message.writer.print("ax ask: failed to attach image \"{s}\": {s}\n", .{ image_path, reason });
...
fn runPromptInternal(alloc: Allocator, prompt: []const u8, permission_override: ?PermissionMode, cfg: Config, options: RunOptions) !PromptRunResult {
    ...
    if (comptime @import("builtin").os.tag != .wasi) {
        if (ctx.providerSet().select(ctx.provider).deferred_usage != null) {
...
fn resolveModelCapabilities(raw_ctx: *anyopaque, _: Allocator, model: []const u8) model_capabilities.ResolveError!model_capabilities.Capabilities {
    const ctx: *AskContext = @ptrCast(@alignCast(raw_ctx));
    const bundle = ctx.providerSet().select(ctx.provider);
...
fn availableModelCapabilities(raw_ctx: *anyopaque, model: []const u8) model_capabilities.Capabilities {
    ...
    return ctx.capability_resolver.available(
        model,
        ctx.providerSet().select(ctx.provider).fallbackModelCapabilities(model),
    );
}
...
fn finishCliPermissionOutcome(
    ...
    switch (outcome.requirement orelse .approval_required) {
        .configured_rule => try writeBlockedActionGuidance(
            ctx,
            "ax ask: permission required by configured rule",
            label,
            "noninteractive_permission_prompt_unavailable",
            "ax ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        ),
        .approval_required => try writeBlockedActionGuidance(
            ctx,
            "ax ask: permission required for tool execution in noninteractive mode",
            label,
            "noninteractive_permission_prompt_unavailable",
            if (permission_mode == .auto)
                "ax ask: human approval is required for this action; use the interactive shell to approve it, or add a narrow matching permission rule\n"
            else
                "ax ask: rerun with --auto to review this exact action automatically, or use the interactive shell to approve it\n",
        ),
    }
...
fn writeBlockedActionGuidance(
    ...
    try ctx.writeStderr("ax ask: blocked action: ");
...
fn emitAskNotificationBell(raw: *anyopaque) void {
    ...
    ctx.writeStderr("\x07") catch |err| {
        debug_trace.logf(
            "notifications",
            "ax ask terminal bell write failed err={s}",
            .{@errorName(err)},
        );
    };
}
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "ax ask: failed to save YOLO acknowledgment: {s}\n",
...
fn promptRealPermissionApproval(
    ...
    try write_stderr(stderr_ctx, "ax wants to run:\n  ");
...
test "ax ask deps validate malformed registered calls" {
    const alloc = std.testing.allocator;
...
test "ax ask ChatGPT route disables Gateway-backed auxiliary providers" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "ax ask finalization fails every failed turn" {
...
test "ax ask deps reject malformed native web_search calls" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "headless yolo warning precedes startup configuration diagnostics" {
    ...
    try std.testing.expect(std.mem.startsWith(
        ...
        stderr_capture.bytes.items,
        permissions.yolo_warning_text ++ "\n" ++
            "ax ask: config user: malformed_settings\n",
    ));
}
...
test "stdin read failure has distinct text and JSON output contracts" {
    ...
    try std.testing.expectEqualStrings(
        "ax ask: failed to read prompt from stdin\n",
        stderr_capture.bytes.items,
    );
...
test "ax ask default user commands require configured authority or review" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "ax ask automatic review observes worker cancellation" {
...
test "ax ask terminal permission prompt approves and denies run_command" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "ax ask permission attention fires once after a prompt is published" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "ax ask bell writes only to TTY stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask registers notification handlers only when configured" {
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(std.testing.allocator);
    var stderr_capture: TestCapture = .{};
    defer stderr_capture.deinit(std.testing.allocator);
...
test "ax ask captured and quiet permission paths bypass terminal prompt" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "ax ask permission prompt policy requires explicit captured-mode opt in and TTY stdin" {
    const Case = struct {
        output_mode: OutputMode,
        prompt_permissions: bool,
        stdin_is_tty: bool,
...
test "ax ask captured permission prompt opt in uses the existing prompter" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "ax ask terminal permission prompt propagates prompt hook errors" {
...
test "ax ask prepared file mutation callback preserves terminal permission prompt" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "ax ask auto mode uses automatic allow for external prepared file mutation" {
    const FakeClassifier = struct {
        calls: usize = 0,
        root_text: []const u8 = "",
...
test "ax ask preserves CLI headless blocker diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    ...
    const expected_configured_stderr = try std.fmt.allocPrint(
        arena,
        "ax ask: permission required by configured rule\n" ++
            "ax ask: blocked action: {s}\n" ++
            "ax ask: reason=noninteractive_permission_prompt_unavailable\n" ++
            "ax ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        .{configured_label},
    );
    try std.testing.expectEqualStrings("", stdout_capture.bytes.items);
...
test "ax ask ordinary multi-target admission preserves deny precedence without diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "ax ask renders one-off resume denial in text and JSON modes" {
    const alloc = std.testing.allocator;
    const cases = [_]struct {
...
test "saved ask classifies unsafe store failure by request mode" {
    ...
    try std.testing.expectEqualStrings(
        "ax ask: warning: session persistence unavailable; error=SessionPathUnsafe; continuing without saving\n",
        stderr_capture.bytes.items,
    );
...
test "ax ask JSON captures HTTP 413 prompt-too-long blocker" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "ax ask: HTTP 413: provider payload rejected") != null);
...
test "ax ask formats restricted-provider HTTP errors without raw JSON" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "ax ask: API access denied · HTTP 403 · Provider: wafer") != null);
...
test "ax ask text and JSON share the selected auth failure facts" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask JSON records permission-denied tool calls as error status" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "ax ask JSON permission-denied capture is best effort under allocation failure" {
    var failing = std.testing.FailingAllocator.init(
        std.testing.allocator,
        .{ .fail_index = 0 },
    );
...
test "ax ask JSON captures parallel tool results without corrupting records" {
    const alloc = std.heap.c_allocator;
...
test "ax ask JSON capture cleans up every allocation failure" {
...
test "ax ask JSON records ask_user_question text for matching assertions" {
    const alloc = std.testing.allocator;
    const records = try alloc.alloc(ToolCallRecord, 1);
    records[0] = .{
...
test "ax ask JSON clips ask_user_question text at a UTF-8 boundary" {
    const alloc = std.testing.allocator;
...
test "json run with missing API key prints diagnostic then final object" {
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings("ax ask: " ++ credentials.missing_credential_message ++ "\n", stderr_capture.bytes.items);
    try std.testing.expectEqualStrings(
...
test "ax ask emits one discovery warning when catalog truncation and a skill read report it" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask carries resolved auto mode and initial registry context into the queued prompt" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default ax ask passes canonical image paths as initial context targets" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    {
...
test "default ax ask omits disabled registry context without gathering" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default ax ask preserves project context gathering error mappings" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask JSON recovery keeps stdout structured and reports progress on stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask JSON preserves partial output on prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ax ask raw output still propagates prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
