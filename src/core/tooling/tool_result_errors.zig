...
test "filesystem access denial JSON preserves recovery details" {
    ...
    for (errors) |err| {
        ...
        try std.testing.expect(std.mem.find(u8, suggestion, "ax permissions") != null);
        if (builtin.os.tag == .macos) {
