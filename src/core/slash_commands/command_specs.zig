...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, "𝒂x");
    ...
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "ax [flags]", width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "ax <command> [...flags] [...args]", width, style, .syntax);
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "𝒂x v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expect(std.mem.find(u8, text, "𝒂x starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "ax <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "ax ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "ax session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "ax status --json") != null);
    try std.testing.expect(std.mem.find(u8, text, "Run `/help` inside an interactive session for slash commands.") != null);
    try std.testing.expect(std.mem.find(u8, text, "Learn more about 𝒂x:  https://fx.sh/docs") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:     run `/feedback` inside 𝒂x\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `ax <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1m𝒂x\x1b[0m"));
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside 𝒂x\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the 𝒂x version and exit\n\nExamples:") != null);
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "ax permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  ax permissions [--json]") != null);
...
test "per-command help preserves long resume usage outside top-level index" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  ax session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
...
test "ACP help documents accepted options" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "ax acp\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  ax acp [--model <id>] [--log-file <path>]") != null);
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("open the ax feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
